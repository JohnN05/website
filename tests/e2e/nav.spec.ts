...
test('desktop: rail renders fully expanded with both links reachable and labeled', async ({ page }) => {
  // 1440px is past the rail's own 1400px full-width threshold (--rail-width
  // in global.css), so it's pinned at its 14rem max here rather than the
...
test('desktop: rail narrows toward its floor on a narrower desktop window', async ({ page }) => {
  // Below the rail's 1400px full-width threshold, --rail-width clamps
  // between a 9rem floor and 14rem — at 900px it should be narrower than
  // 14rem but no narrower than the 9rem (144px) floor, and still above the
  // 769px breakpoint where the rail disappears entirely for the mobile nav.
